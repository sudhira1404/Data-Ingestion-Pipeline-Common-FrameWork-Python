...
#sh -x /home/corp.target.com/svmdedmp/file_ingestion/wrapper/file_ingest_wrapper_no_venv.sh NSA sftp_upload.py config_file_ingestion file_ingest_wrapper_param_no_venv.sh local y
#for only upload to hdfs
#sh -x /home/corp.target.com/svmdedmp/file_ingestion/wrapper/file_ingest_wrapper_no_venv.sh NSA_upload upload.py config_file_ingestion file_ingest_wrapper_param_no_venv.sh local y
#for only td s3 upload to hdfs
#sh -x /home/corp.target.com/svmdedmp/file_ingestion/wrapper/file_ingest_wrapper_no_venv.sh td_s3_upload td_s3_upload.py config_td_s3 file_ingest_wrapper_param_no_venv.sh local y
...
pythonwrappermodule=`echo "${pythonwrapper%%.*}"`
python -m ingest.wrapper.$pythonwrappermodule -c $jobname  -d $config_filepath -e $env > $logfile && exit_status=`echo 0` || exit_status=`echo 1`
