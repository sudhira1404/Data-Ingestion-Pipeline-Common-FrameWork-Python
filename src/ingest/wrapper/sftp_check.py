...
#from service import sftp_xenon
#from utility import util

from ingest.service import sftp_xenon
...
if __name__ == "__main__":
    ...
    with open(file_name, 'r') as ymlfile:
        ...
        local_sftp_temp_file_path = cfg[section_name]['local_sftp_temp_file_path']
    ...
    filename = log_filename + 'FW' + job_start + ".log"
    logfilename = log_filepath + "/" + filename
    util.logs(logfilename)

    sftp = sftp_xenon.Sftp_Xenon(xenon_url,xenon_user,xenon_password,xenon_method,edge_node,remote_host,remote_username,local_private_key,sftp_method,method,remote_passwordauth,remoteuser_password,platform)
    sftp.sftp_check(remote_file_path, local_file_path, remote_file_regex_pattern ,remote_ctrlfile_regex_pattern,
                   hdfs_sftp_filepath, hdfs_sftp_filename,local_hdfs_sftp_file_path,local_hdfs_sftp_temp_file_path,platform,local_sftp_temp_file_path+'/'+section_name,section_name+'.txt')
    sftp.conn.close()